function [y_predict,pmask_all]=cpm_cv_pca(x,y,pthresh,kfolds)
...
pmask_all=zeros(size(x,1),kfolds); % Jean add
...
for leftout = 1:kfolds
    ...
    [y_predict(testinds),pmask_all(:,leftout)] = cpm_test(x_test,mdl,pmask); % Jean updated to save mask
end
